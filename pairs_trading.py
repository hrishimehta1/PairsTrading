...
def calculate_spreads(stock_1_close_prices, stock_2_close_prices, pair_daily_ratios, average_ratio):
    pair_daily_spreads = []
    for x in range(0, len(pair_daily_ratios)):
        days_spread = ((stock_1_close_prices[x]*average_ratio) - stock_2_close_prices[x])
...
def calculate_r_squared_correlation(stock_1, stock_2, stock_1_close_prices, stock_2_close_prices):
    try:
        corr_matrix = numpy.corrcoef(stock_1_close_prices, stock_2_close_prices)
        corr = corr_matrix[0,1]
        r_sq = corr**2
        return r_sq
    except:
        print("error: ", "error calculating R_squared")
        with open('error_log.txt', 'a') as f:
            f.write(f'{stock_1} {stock_2} error calculating R_squared')
            f.write('\n') 
        return
...
def pairs_trade(ticker_a, ticker_b): 
    try:
